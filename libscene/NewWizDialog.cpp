...
void CNewWizDialog::AddPage(CNewWizPage* pPage, UINT nID)
{
	...
	pPage->m_nIndex = (int)m_PageList.GetCount();
}
...
void CNewWizDialog::SetScene(const Scene* pScene)
{
	m_Scene = *pScene;
}

Scene* CNewWizDialog::GetScene()
{
	return &m_Scene;
}

void CNewWizDialog::SetSceneList(const SceneList* pSceneList)
{
	m_SceneList = *pSceneList;
}

SceneList* CNewWizDialog::GetSceneList()
{
	return &m_SceneList;
}

BOOL CNewWizDialog::SetPicture(const char* pszPicPathName)
{
	if (_access_s(pszPicPathName, 0))
		return FALSE;
	wchar_t wpath[MAX_PATH] = {0};
	MultiByteToWideChar(pszPicPathName, wpath, MAX_PATH);
	if (m_pSceneImage)
	{
		delete m_pSceneImage;
		m_pSceneImage = NULL;
	...
	m_pSceneImage = Gdiplus::Bitmap::FromFile(wpath);

	if(m_pSceneImage)
	{
	if (m_pSceneImage->GetLastStatus() == Gdiplus::Ok)
		return TRUE;
	delete m_pSceneImage;
	}
	m_pSceneImage = NULL;
...
Gdiplus::Bitmap* CNewWizDialog::GetPicture()
{
	return m_pSceneImage;
}

BOOL CNewWizDialog::GetPicBySceneID(CString& strPicPathName, const char* pSceneID)
{
	SceneList::const_iterator itrScene = m_SceneList.begin();
	for (; itrScene != m_SceneList.end(); ++itrScene)
	{
		if (itrScene->sID == pSceneID)
		{
			strPicPathName.Format(_T("%s\\%s"), m_strCachePath.GetString(), itrScene->sPic.c_str());
			return TRUE;
		}
	}
