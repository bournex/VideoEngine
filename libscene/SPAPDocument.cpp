...
//------------------------//
inline _bstr_t ConvertStringToBSTR(const char* pSrc)
{
	if(!pSrc) return L"";
...
{
	_bstr_t bstDocContent = m_docPtr->Getxml();

	return bstDocContent;
}

int CSPAPDocument::GetDocXmlLen() const
{
	int len = (int)SysStringLen(m_docPtr->Getxml());
	return len;
}

void CSPAPDocument::GetDocXml(char* pXml, int bufLen)
{
	string xml = ConvertBSTRToString(m_docPtr->Getxml());
	if (bufLen > (int)xml.length())
		strcpy(pXml, xml.c_str());
}
...
BOOL CSPAPDocument::LoadFromXML(LPCTSTR cszXML)
{
	...
	try
	{
		...
		_bstr_t bstr = ConvertStringToBSTR(cszXML);
		VARIANT_BOOL vr = m_docPtr->loadXML(bstr);
		if ( vr == VARIANT_FALSE )
			bOK = FALSE;
	}
	catch (...)
...
BOOL CSPAPDocument::SetCDATASectionData(MSXML2::IXMLDOMNodePtr nodeDest, LPCTSTR cszText)
{
	if( NULL==nodeDest || NULL==cszText)
	{
		return FALSE;
	...
	try
	{
		MSXML2::IXMLDOMCDATASectionPtr cdata = m_docPtr->createCDATASection(cszText);
		nodeDest->appendChild(cdata);
	}
	catch (...)
	{
		ShowError(_T("XML access failed : set cdata section data!"));
		bOK = FALSE;	
	}
...
_bstr_t  CSPAPDocument::GetCDATASectionData(MSXML2::IXMLDOMNodePtr nodeFrom)
{
	if (nodeFrom == NULL)
		return "";
	...
	try
	{
		return nodeFrom->Gettext();
	}
	catch(...)
	{
		ShowError(_T("XML access failed  - GetCDATASectionData!"));
	}
