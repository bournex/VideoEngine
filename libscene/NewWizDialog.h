...
protected:
	...
	Scene m_Scene;		// creating scene
	SceneList m_SceneList;	// Scene List
	Gdiplus::Bitmap* m_pSceneImage;
	CString m_strCachePath;
	CString m_strInfoEx; // ∏Ωº”–≈œ¢
...
public:
  ...
  void SetNextPage();
	...
	//////////////////////////////////////////////////////////////////////////
	void SetCachePath(const char* path) { m_strCachePath = path; }
	void GetCachePath(CString& strPath) { strPath = m_strCachePath; }
	void SetScene(const Scene* pScene);
	Scene* GetScene();
	void SetSceneList(const SceneList* pSceneList);
	SceneList* GetSceneList();
	BOOL SetPicture(const char* pszPicPathName);
	Gdiplus::Bitmap* GetPicture();
	BOOL GetPicBySceneID(CString& strPicPathName, const char* pSceneID);
	void SetInfoEx(const char* pszInfoEx) { m_strInfoEx = pszInfoEx; }
	void GetInfoEx(CString& strInfoEx) { strInfoEx = m_strInfoEx; }
