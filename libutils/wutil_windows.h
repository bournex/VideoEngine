#ifndef __WUTIL_WINDOWS_H__2012_09_06__
#define __WUTIL_WINDOWS_H__2012_09_06__
...
// UTF-8转成GB2312编码
static BOOL UTF8ToGB(LPCSTR pszSrc, LPTSTR pszDst, int nDstSize)   
{   
	WCHAR *pszTemp;

	int i = MultiByteToWideChar(CP_UTF8, 0, pszSrc, -1, NULL, 0);
	i = i<<1;
	pszTemp = (WCHAR*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, i);
	if (pszTemp == NULL)
		return FALSE;

	MultiByteToWideChar(CP_UTF8, 0, pszSrc, -1, pszTemp, i); 

	WideCharToMultiByte(CP_ACP, 0, pszTemp, -1, pszDst, nDstSize, NULL, NULL);  
...
// GB2312转成UTF-8编码
static BOOL GBToUTF8(LPCSTR pszSrc, LPSTR pszDst, int nDstSize)   
{   
	WCHAR *pszTemp;

	int i = MultiByteToWideChar(CP_ACP, 0, pszSrc, -1, NULL, 0);
	i = i<<1;
	pszTemp = (WCHAR*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, i);
	if (pszTemp == NULL)
		return FALSE;

	MultiByteToWideChar(CP_ACP, 0, pszSrc, -1, pszTemp, i); 

	WideCharToMultiByte(CP_UTF8, 0, pszTemp, -1, pszDst, nDstSize, NULL, NULL);  
...
NAMESAPCE_BEGIN(netutil)

static BOOL GetLocalIPAddress(char ipAddress[64][64], int* nCount)
{
	char szHostName[128];
	if (gethostname(szHostName, 128) == 0)
	{
		...
		if (host)
		{
			...
			for (; host->h_addr_list[i]; ++i)
			{
				char a[64] = {0};
				strcpy(ipAddress[i],"");
				for (int j=0; j<host->h_length; ++j)
				{
					if (j > 0)
						strcat(ipAddress[i], ".");
					sprintf(a, "%u", (unsigned int)((unsigned char*)host->h_addr_list[i])[j]);
					strcat(ipAddress[i], a);
				}
			}
